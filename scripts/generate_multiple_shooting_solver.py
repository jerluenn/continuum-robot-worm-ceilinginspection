...
class Multiple_Shooting_Solver:
    ...
    def create_static_solver_position_boundary(self): 
        ...
        self.ocp.constraints.lbx_0 = np.hstack((-np.ones(3)*self.pos_ub, -np.ones(4)*self.eta_ub, self.wrench_lb*np.ones(6), np.zeros(self.nx-13)))

        self.ocp.constraints.ubx_0 = np.hstack((np.ones(3)*self.pos_ub, np.ones(4)*self.eta_ub, self.wrench_ub*np.ones(6), np.ones(self.nx-13)*self.tension_max))   
        ...
        self.ocp.constraints.lbx_e = np.hstack((np.array([0, 0, 0, 1, 0, 0, 0]), self.wrench_lb*np.ones(6), np.zeros(self.nx-13)))

        self.ocp.constraints.ubx_e = np.hstack((np.array([0, 0, 0, 1, 0, 0, 0]), self.wrench_ub*np.ones(6), np.ones(self.nx-13)*self.tension_max))
    ...
    def create_static_solver_full_robot_1(self): 
        ...
        self.ocp.constraints.lbx_0 = np.hstack((0, 0, 0, -self.eta_ub, -self.eta_ub, -self.eta_ub, -self.eta_ub, self.wrench_lb*np.ones(6), np.zeros(self.nx - 13)))

        self.ocp.constraints.ubx_0 = np.hstack((0, 0, 0, self.eta_ub, self.eta_ub, self.eta_ub, self.eta_ub, self.wrench_ub*np.ones(6), np.zeros(self.nx - 13)))        
    ...
    def create_static_solver_full_robot_0(self): 
